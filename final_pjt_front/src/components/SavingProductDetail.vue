...
<template>
    <div class="card-overlay" @click.self="closeCard">
      <div class="card">
        ...
        <hr>
        <CharSaving :product="product"></CharSaving>
  ...
  import CharSaving from './CharSaving.vue';
...
const getMyLoveProducts = async function() {
  try {
    ...
    const lovedProductsResponse = await axios({
      method: 'get',
      url: `http://localhost:8000/liked-products/saving/`,
...
const getpropslovecount = async function() {
  try {
    ...
    const response = await axios({
      method: 'get',
      url: `http://localhost:8000/product-likes/saving/${props.product.fin_prdt_cd}/`,
      headers: { Authorization: `Token ${token}` },
    });
...
const likeProduct = function(){
  ...
  axios({
    ...
    const togg = await savingStore.toggleLove(props.product.fin_prdt_cd)
...
const saveProduct = async function () {
  ...
  swal({
    ...
    if (willSave) {
      if (isJjim.value) {
        // 찜 취소
        await savingStore.unjjim_saving(props.product.fin_prdt_cd);
      } else {
        // 찜 추가
        await savingStore.jjim_saving(props.product.fin_prdt_cd);
      }
