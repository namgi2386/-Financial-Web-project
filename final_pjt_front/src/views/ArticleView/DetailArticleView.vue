<template>
    <div v-if="articleStore.Article && userStore.profileData">
      <!-- 뒤로 가기 버튼 -->
      <!-- <button class="returnBtn" @click="returnArticleList">← Back</button> -->
      ...
      <div class="article-card">
        ...
        <hr />
        <!-- 댓글 컴포넌트 -->
        <Comments v-if="articleStore.Article && userStore.profileData" :Username="userStore.profileData.nickname" :articleUser="articleStore.Article.user" @commentAdded="handleCommentAdded() "/>
  ...
  const articleDelete = () => {
    swal({
      ...
      if (willDelete) {
        articleStore.DeleteDetailArticle('free',articleStore.Article.id);
        swal("성공적으로 삭제 되었습니다!", {
          icon: "success",
        });
...
const saveComment = async (reco) => {
  ...
  if (editedContent.value.trim()) {
    try {
      // 서버에 수정된 댓글 내용 전송
      await commentStore.UpdateRecommend('free', reco, editedContent.value);
...
const cancelEdit = () => {
  ...
  await commentStore.DeleteRecommend('free', recoId);
