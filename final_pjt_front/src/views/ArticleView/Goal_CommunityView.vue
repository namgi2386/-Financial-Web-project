<template>
  <div class="container">
    <div class="text-center main-title pb-0">
      <h1 class="title">목표공유 게시판</h1>
    </div>
    <div class="card my-4 mx-auto" style="max-width: 100%;">
      <div class="row justify-end">
        <div cols="auto" class="d-flex justify-content-end">
          <router-link :to="{name: 'goal_createarticle'}" class="mt-5 btn-new-post">New GOAL</router-link>
        </div>
      </div>
...
onMounted(async () => {
  try {
    await articleStore.ReadArticles('goal'); // 비동기 호출
...
const detailView = (articleId, nickname, img , creatorId) => {
  router.push({ name: "goal_detailarticle", params: { id: articleId }, query: { nickname: nickname, img: img , creatorId : creatorId } });
};
</script>
