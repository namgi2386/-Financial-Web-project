<template>
  <div v-if="articleStore.Article && userStore.profileData">
    ...
    <div class="article-card">
      ...
      <hr />
      <!-- 댓글 컴포넌트 -->
      <qna_Comments v-if="articleStore.Article && userStore.profileData" :Username="userStore.profileData.nickname" :articleUser="articleStore.Article.user"  @commentAdded="handleCommentAdded() "/>
      
      
      
      <ul >
        <div v-for="reco in articleStore.ArticlesComments" :key="reco.id">
          <!-- {{ userStore.getProfileDetail(reco.recommend_user) }} -->
          <li>
            <!-- 수정 모드인지 확인 -->
...
import qna_Comments from "@/components/qna_Comments.vue";
...
const articleDelete = () => {
  swal({
    ...
    if (willDelete) {
      articleStore.DeleteDetailArticle('qna',articleStore.Article.id);
      swal("성공적으로 삭제 되었습니다!", {
        icon: "success",
      });
      // communityView.vue로 이동
      router.push({ name: "qna_community" });
    } else {
      swal("취소되었습니다!");
    }
  });
...
const saveComment = async (reco) => {
  ...
  if (editedContent.value.trim()) {
    try {
      // 서버에 수정된 댓글 내용 전송
      await commentStore.UpdateRecommend('qna', reco, editedContent.value);
...
onMounted(async() => {
  await articleStore.ReadDetailArticle('qna', route.params.id);
  // console.log(articleStore.Article);
  // console.log('ddddd');
  
  // await commentStore.GETRecommend('qna',articleStore.Article.id)
});
...
const removeComment = async (recoId) => {
  await commentStore.DeleteRecommend('qna', recoId);
