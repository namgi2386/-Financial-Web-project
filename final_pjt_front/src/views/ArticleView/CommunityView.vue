    ...
    <div class="card my-4 mx-auto" style="max-width: 100%;">
      <div class="row justify-end">
        <div cols="auto" class="d-flex justify-content-end">
          <router-link :to="{name: 'createarticle'}" class="mt-5 btn-new-post">New POST</router-link>
        </div>
      </div>
...
onMounted(async () => {
  try {
    await articleStore.ReadArticles('free'); // 비동기 호출
...
const detailView = (articleId, nickname, img , creatorId) => {
  router.push({ name: "detailarticle", params: { id: articleId }, query: { nickname: nickname, img: img , creatorId : creatorId } });
};
</script>
